...
contract TransactionFeesOracle is Controlled, ITransactionFeesOracle, EtherTokenConstant {
    ...
    */
    function setFees(bytes32[] calldata _appIds, ERC20[] calldata _tokens, uint256[] calldata _amounts) external onlyConfigGovernor {
        ...
        for (uint256 i = 0; i < _appIds.length; i++) {
            _setFee(_appIds[i], _tokens[i], _amounts[i]);
        }
    }
    ...
    * @param _appId Id of the app
    */
    function unsetFee(bytes32 _appId) external onlyConfigGovernor {
        _unsetFee(_appId);
    }
    ...
    */
    function unsetFees(bytes32[] calldata _appIds) external onlyConfigGovernor {
        for (uint256 i = 0; i < _appIds.length; i++) {
            _unsetFee(_appIds[i]);
        }
    }
    ...
    */
    ...
    function _setFee(bytes32 _appId, ERC20 _token, uint256 _amount) internal {
    ...
    function _unsetFee(bytes32 _appId) internal {
