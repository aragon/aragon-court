  ...
  context('activating jurors', () => {
    ...
    context('on dispute', () => {
      ...
      it('fails to draft outside of the draft term', async () => {
        ...
        await assertRevert(this.court.draft(disputeId, jurorsNumber), ERROR_NOT_DRAFT_TERM)
      })
      ...
      context('on juror draft (hijacked)', () => {
        ...
        beforeEach(async () => {
          ...
          const receipt = await this.court.draft(disputeId, jurorsNumber)
        ...
        it('fails to draft a second time', async () => {
          await assertRevert(this.court.draft(disputeId, jurorsNumber), ERROR_ROUND_ALREADY_DRAFTED)
        })
            ...
            context('on appeal', () => {
              ...
              context('on appeal confirmation', () => {
                ...
                const draftJurors = async () => {
                  ...
                  const receipt = await this.court.draft(disputeId, 1000)
                  assertDeepLogs(receipt, this.jurorsRegistry.abi, JUROR_DRAFTED_EVENT)
                  assertLogs(receipt, DISPUTE_STATE_CHANGED_EVENT)
                }
