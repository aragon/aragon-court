...
contract('Court: final appeal', ([ poor, rich, governor, juror1, juror2, juror3, juror4, juror5, juror6, juror7 ]) => {
  ...
  const ERROR_INVALID_ADJUDICATION_STATE = 'CTINVALID_ADJUDICATION_STATE'
  const ERROR_INVALID_ADJUDICATION_ROUND = 'CTINVALID_ADJUDICATION_ROUND'
  const ERROR_FINAL_ROUNDS_PENDING = 'CTFINAL_ROUNDS_PENDING'
    ...
    context('Rewards and slashes', () => {
      ...
      it('losers jurors have penalty', async () => {
        for (let i = winningJurors; i < initialJurorNumber; i++) {
          ...
          assert.equal(courtBalance + weightedPenalty, (await this.staking.totalStakedFor(jurors[i])).toNumber(), `balance in court doesn't match for juror ${i}`)
        }
      })
    })
  })
