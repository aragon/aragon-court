  ...
  describe('set and get fees', () => {
    ...
    const setAndGetAppFees = (isEth) => {
      ...
      it('fails to set multiple fees if amounts length doesn’t match', async () => {
        await assertRevert(aragonAppFeesCashier.setAppFees(appIds, [token, token], [1], { from: governor }), SUBSCRIPTIONS_ERRORS.WRONG_AMOUNTS_LENGTH)
      })
    ...
    context('when the app fee hasn’t been set', () => {
      ...
      context('when the token is not ETH', () => {
        it('fails to set fee if token is not contract', async () => {
          await assertRevert(aragonAppFeesCashier.setAppFee(VOTING_APP_ID, fakeToken, 1, { from: governor }), SUBSCRIPTIONS_ERRORS.WRONG_TOKEN)
        })

        it('fails to set multiple fees if token is not contract', async () => {
          await assertRevert(aragonAppFeesCashier.setAppFees([VOTING_APP_ID, TOKEN_MANAGER_APP_ID], [fakeToken, feeToken.address], [1, 1], { from: governor }), SUBSCRIPTIONS_ERRORS.WRONG_TOKEN)
          await assertRevert(aragonAppFeesCashier.setAppFees([VOTING_APP_ID, TOKEN_MANAGER_APP_ID], [feeToken.address, fakeToken], [1, 1], { from: governor }), SUBSCRIPTIONS_ERRORS.WRONG_TOKEN)
        })
