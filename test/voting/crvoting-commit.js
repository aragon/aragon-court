...
const { DISPUTES_MANAGER_ERRORS, VOTING_ERRORS } = require('../helpers/utils/errors')
      ...
      context('when the voter has not voted before', () => {
        context('when the owner does not revert when checking the weight of the voter', () => {
          ...
          context('when the owner tells a zeroed weight', () => {
            ...
            it('reverts', async () => {
              await assertRevert(voting.commit(voteId, '0x', { from: voter }), DISPUTES_MANAGER_ERRORS.VOTER_WEIGHT_ZERO)
            })
          })
        })
