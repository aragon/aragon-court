...
const DisputesManager = artifacts.require('DisputesManagerMockForVoting')
    ...
    context('when the sender is the owner', () => {
      ...
      context('when the given vote ID was not used before', () => {
        context('when the given possible outcomes is valid', () => {
          ...
          it('creates the given voting', async () => {
            await disputesManager.create(voteId, possibleOutcomes)
          ...
          it('emits an event', async () => {
            const receipt = await disputesManager.create(voteId, possibleOutcomes)
        ...
        context('when the possible outcomes below the minimum', () => {
          it('reverts', async () => {
            await assertRevert(disputesManager.create(voteId, 0), VOTING_ERRORS.INVALID_OUTCOMES_AMOUNT)
            await assertRevert(disputesManager.create(voteId, 1), VOTING_ERRORS.INVALID_OUTCOMES_AMOUNT)
          })
        })

        context('when the possible outcomes above the maximum', () => {
          it('reverts', async () => {
            await assertRevert(disputesManager.create(voteId, 510), VOTING_ERRORS.INVALID_OUTCOMES_AMOUNT)
          })
        })
      })
      ...
      context('when the given vote ID was already used', () => {
        beforeEach('create voting', async () => {
          await disputesManager.create(voteId, 2)
        })

        it('reverts', async () => {
          await assertRevert(disputesManager.create(voteId, 2), VOTING_ERRORS.VOTE_ALREADY_EXISTS)
        })
      })
    })
